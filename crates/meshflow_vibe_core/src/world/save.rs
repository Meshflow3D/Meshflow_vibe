...
pub struct WorldState {
    ...
    pub entity_data: Option<
        Vec<(
            Entity,
            IdentityData,
            Transform,
            Option<Entity>,
            crate::entities::SaveSettings,
        )>,
    >, // Added parent entity, UUID, and SaveSettings
...
pub fn save_data_ready_system(
    ...
    for RuntimeDataReadyEvent(source) in event_reader.read() {
        ...
        if let Some((path, world_state)) = save_request_data.pending_saves.remove(source.as_str()) {
