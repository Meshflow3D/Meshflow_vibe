    ...
    fn spawn(
        ...
        if self.has_atmosphere {
            if let Some(atmos_settings) = &self.atmosphere_settings {
                ...
                // NOTE: Bevy 0.18 Atmosphere requires Handle<ScatteringMedium> but Handle creation API changed
                // Using placeholder - proper implementation requires AssetServer to load ScatteringMedium asset
                entity.insert(Hdr);
                // entity.insert(atmosphere); // TODO: Fix Handle<ScatteringMedium> creation
