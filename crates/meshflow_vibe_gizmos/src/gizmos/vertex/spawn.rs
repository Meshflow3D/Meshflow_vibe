...
use crate::gizmos::GizmoType;
use crate::gizmos::NewGizmoType;
...
use meshflow_vibe_core::{EditorIgnore, TreeHiddenEntity, UICamera};
...
pub fn spawn_vertex_visualizations(
    ...
    for (entity, mesh3d) in selected_entities.iter() {
        ...
        let shared_mesh = meshes.add(Sphere::new(config.vertex_size).mesh().ico(2).unwrap());
        ...
        let _selected_material = if let Some(mat) = &config.selected_material {
            mat.clone()
        } else {
            let mat = materials.add(StandardMaterial {
