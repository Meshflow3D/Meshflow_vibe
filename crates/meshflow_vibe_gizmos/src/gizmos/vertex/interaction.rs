...
pub fn handle_vertex_click(
    ...
    let is_additive = user_input.current_button_inputs.iter().any(|input| {
        matches!(
            input,
            meshflow_vibe_core::InputTypes::Button(KeyCode::ShiftLeft | KeyCode::ShiftRight)
        )
    });
...
pub fn update_vertex_colors(
    ...
    mut selected_vertices: Query<
        &mut MeshMaterial3d<StandardMaterial>,
        (With<SelectedVertex>, With<VertexMarker>),
    >,
    mut unselected_vertices: Query<
        &mut MeshMaterial3d<StandardMaterial>,
...
pub fn deselect_all_vertices(
    ...
    let should_deselect = user_input.current_button_inputs.iter().any(|input| {
        matches!(
            input,
            meshflow_vibe_core::InputTypes::Button(KeyCode::Escape)
        )
    });
