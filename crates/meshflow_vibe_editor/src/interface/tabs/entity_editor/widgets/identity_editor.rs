...
use meshflow_vibe_core::{
    entities::GraniteType, ClassCategory, EditableMaterial, GraniteTypes, NewEditableMaterial,
};
...
fn display_class_specific_data(ui: &mut egui::Ui, tab_data: &mut EntityEditorTabData) {
    ...
    ui.vertical(|ui| {
        ...
        if matches!(class_data.category(), ClassCategory::Mesh) {
...
fn display_material_data(
    ...
    ui.collapsing("Material Properties", |ui| {
        ...
        ui.horizontal(|ui| {
            ...
            if delete_clicked
                && !current_material.is_empty()
                && current_material.friendly_name != "None"
            {
