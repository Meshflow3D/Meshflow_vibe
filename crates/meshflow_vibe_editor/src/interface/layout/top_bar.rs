    ...
    ui.vertical(|ui| {
        ...
        ui.horizontal(|ui| {
            ...
            ui.menu_button("Panels", |ui| {
                for (tab_type, label) in [
                    ...
                    let tab = side_dock
                        .dock_state
                        .find_tab_from(|tab| tab.get_type() == tab_type);
        ...
        ui.horizontal(|ui| {
            ...
            ui.menu_button("Viewport Camera", |ui| {
                ...
                if ui.selectable_label(using_editor, "Editor Camera").clicked() && !using_editor {
                    events
                        .viewport_camera
                ...
                if camera_options.is_empty() {
                    ...
                    for (entity, label) in camera_options.iter() {
                        let is_active = viewport_camera_state.active_override == Some(*entity);
            ...
            ui.separator();
            if !edit_session.is_active() && ui.button("Enter Edit Mode").clicked() {
                if let Some((entity, topology_owner)) = active_selection_with_topology.iter().next()
                {
            ...
            ui.separator();
            if edit_session.is_active() && ui.button("Exit Edit Mode").clicked() {
                events
                    .exit_edit_mode
